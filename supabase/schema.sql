...
-- ============================================================
-- 사진 인증 (2026-07-10) — SQL 불필요
-- 사진은 Edge Function `fm-photo`가 처리한다 (supabase/functions/fm-photo/).
--  · POST: participants.token 검증 후 service role로 Storage 업로드
--  · GET:  {challengeId}/{date}/ 경로 목록 → 피드
--  · 버킷 fitmate-photos(공개, 1MB, jpeg)는 최초 업로드 시 자동 생성
-- 배포: supabase functions deploy fm-photo --project-ref wzlapxdnfhgapheuuqnl --no-verify-jwt
